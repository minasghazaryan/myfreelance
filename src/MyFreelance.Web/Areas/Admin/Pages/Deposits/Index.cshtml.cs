...
public class IndexModel(ApplicationDbContext db, IDepositService depositService, UserManager<ApplicationUser> userManager) : PageModel
{
    ...
    public record DepositItem(Guid Id, string UserEmail, decimal Amount, string? TransactionHash, string Status, DateTime CreatedAt);

    public async Task OnGetAsync()
    {
        Deposits = await db.Deposits.Include(d => d.User).OrderByDescending(d => d.CreatedAt).Take(100)
            .Select(d => new DepositItem(d.Id, d.User.Email!, d.Amount, d.TransactionHash, d.Status.ToString(), d.CreatedAt)).ToListAsync();
    }
